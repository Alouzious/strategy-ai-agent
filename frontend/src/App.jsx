...
/* ─────────────────────────────────────────
   SMART PARSER — handles text + markdown tables
───────────────────────────────────────── */
function parseContent(text) {
  ...
  while (i < lines.length) {
    ...
    if (isHeading) {
      blocks.push({
        type: "heading",
        text: line.replace(/^#+\s*/,"").replace(/\*\*/g,"").replace(/^\d+\.\s*/,""),
      });
      i++; continue;
    }
              ...
              if (b.type === "table") return (
                <div key={j} className="rv-table-wrap">
                  <table className="rv-table">
                    <thead>
                      <tr>{b.headers.map((h,k)=><th key={k}>{h}</th>)}</tr>
                    </thead>
                    <tbody>
                      {b.rows.map((row,k)=>(
...
function exportPDF(result) {
  ...
  doc.text(`Industry: ${result.industry} | Audience: ${result.audience}`,margin,y);
  ...
  AGENTS.forEach((agent,idx)=>{
    ...
    parseContent(text).forEach(b=>{
      if(b.type==="heading"){checkPage(22);doc.setFontSize(10);doc.setFont("helvetica","bold");doc.setTextColor(160,160,220);doc.text(b.text,margin,y);y+=15;}
        ...
        y+=14;
        b.rows.forEach(row=>{
          checkPage(12);doc.setFont("helvetica","normal");doc.setTextColor(150,150,210);
          row.forEach((cell,k)=>{doc.text(String(cell).slice(0,20),margin+k*colW,y);});
          y+=12;
        });
        y+=6;
      }
...
export default function App() {
  ...
  const [history,setHistory]=useState(()=>{try{return JSON.parse(localStorage.getItem("strategy_history")||"[]");}catch{return [];}});
  ...
  const saveToHistory=(r)=>{
    ...
    setHistory(updated);localStorage.setItem("strategy_history",JSON.stringify(updated));
  };
  ...
  const runAgents=async()=>{
    ...
    try{
      ...
      setError(msg);updateToast(tid,msg,"error");
    }
    setActive("");setLoading(false);
  };

  const handleExample=(ex)=>{setTopic(ex.topic);setIndustry(ex.industry);setAudience(ex.audience);setPage("workspace");addToast("Example loaded — click Generate to run!","info");};
  const handleExport=()=>{if(!result)return;const tid=addToast("Generating PDF...","loading");setTimeout(()=>{try{exportPDF(result);updateToast(tid,"PDF downloaded!","success");}catch{updateToast(tid,"PDF export failed.","error");}},300);};
  const loadHistory=(entry)=>{setResult(entry.result);setTopic(entry.topic);setIndustry(entry.industry);setAudience(entry.audience);setPage("workspace");setExpanded("writer");addToast("Report loaded from history.","info");};
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {AGENTS.map((agent,i)=>{
            ...
            return(<div key={agent.key} className={`pipeline-step ${isActive?"running":""} ${isPast?"done":""}`}><div className="pipeline-dot" style={{background:agent.color}}/><span>{agent.label}</span>{isActive&&<Loader2 size={11} className="spin"/>}{isPast&&<CheckCircle2 size={11} style={{color:agent.color}}/>}</div>);
          })}
        </nav>
        <div className="sidebar-foot">
          <button className="nav-item"><Settings size={15}/><span>Settings</span></button>
          <div className="sidebar-user"><div className="user-avatar">U</div><div><span className="user-name">User</span><span className="user-plan">Pro Plan</span></div></div>
        </div>
      </aside>
      ...
      <div className="main-wrapper">
        <header className="topbar">
          <div className="topbar-left">
            ...
            <div className="breadcrumb"><span>Platform</span><ChevronRight size={13}/><span className="bc-active">{page==="home"?"Dashboard":page==="history"?"History":"Strategy Builder"}</span></div>
          </div>
        ...
        <main className="main">
          {page==="home"&&(
            <div className="page-home">
              ...
              <div className="stats-strip">
                {[["5","Pipeline Stages"],["Tables","& Charts"],["SWOT","Analysis"],["800+","Word Reports"],["PDF","Export"]].map(([v,l])=>(
              ...
              <div className="hiw-section">
                ...
                <div className="hiw-grid">{HOW_IT_WORKS.map((h,i)=>(
                  <div key={i} className="hiw-card"><div className="hiw-num">0{i+1}</div><div className="hiw-icon"><h.Icon size={20}/></div><div className="hiw-title">{h.title}</div><div className="hiw-desc">{h.desc}</div></div>
                ))}</div>
              </div>
              <div className="stages-section">
                ...
                <div className="stages-grid">{AGENTS.map((a,i)=>(
                  <div key={a.key} className="stage-card" style={{"--c":a.color}}>
                    ...
                    <div className="stage-label">{a.label}</div><div className="stage-role">{a.role}</div><div className="stage-desc">{a.desc}</div>
                  </div>
                ))}</div>
              </div>
                ...
                <div className="workspace-split">
                  <div className="ws-left">
                    <div className="ws-card">
                      ...
                      {loading&&(<div className="progress-wrap"><div className="progress-header"><span className="progress-stage">Running: {active} stage</span><span className="progress-pct">{progress}%</span></div><div className="progress-track"><div className="progress-fill" style={{width:`${progress}%`}}/></div></div>)}
                    ...
                    <div className="ws-examples">
                      ...
                      <div className="ws-ex-list">{EXAMPLES.slice(0,3).map((ex,i)=>(
                  ...
                  <div className="ws-right">
                    <div className="pipeline-preview">
                      ...
                      {AGENTS.map((a,i)=>{
                        ...
                        return(<div key={a.key} className={`pp-step ${isActive?"active":""} ${isPast?"past":""}`} style={{"--c":a.color}}>
                          ...
                          <div className="pp-state">{isActive&&<Loader2 size={13} className="spin" style={{color:a.color}}/>}{isPast&&<CheckCircle2 size={13} style={{color:a.color}}/>}{!isActive&&!isPast&&<div className="pp-idle"/>}</div>
                        </div>);
                      })}
                    </div>
                  </div>
                  ...
                  <div className="final-card">
                    <div className="final-card-head">
                      <div className="final-title-wrap">
                        ...
                        <div><div className="final-title">Executive Strategy Report</div><div className="final-sub">Complete synthesized report · {result.topic}</div></div>
                      </div>
          ...
          {page==="history"&&(
            <div className="page-history">
              ...
              {history.length===0?(
                <div className="empty-state"><History size={40} className="empty-icon"/><div className="empty-title">No reports yet</div><div className="empty-sub">Generate your first strategy report to see it here.</div><button className="btn-primary" onClick={()=>setPage("workspace")}><Play size={14}/> Start Building</button></div>
              ):(
                <div className="history-grid">{history.map(entry=>(
                  <div key={entry.id} className="history-card">
                    ...
                    <div className="hc-meta"><span><Building2 size={11}/>{entry.industry}</span><span><Users size={11}/>{entry.audience}</span><span><Clock size={11}/>{entry.time_taken}</span></div>
